...
class risk:
    ...
    def es_backtesting(self,y_pred,k,K=1000,conf_level = 0.05):
        """
        matrix: ES Matrix
        ...
        # generate y_t series
        es = self.es_mat[k][1]
        x_t = [x for x in self.return_gen if x < es]
        vol = minus_fix(y_pred)[-1]
        y_t = (x_t-es)/np.sqrt(vol)
        var_conf_level = self.es_mat[k][0]
    ...
    def es_backtesting(self,y_pred,k,K=1000,conf_level = 0.05):
        ...
        return {"y_t mean":y_t.mean(),"P Value":p_val, 
              "null hypothesis": f"Probability of failure is {round(var_conf_level,3)}",
              "result":result}